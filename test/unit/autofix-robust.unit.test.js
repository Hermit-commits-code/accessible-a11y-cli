...
describe('AccessibilityChecker.autofix (unit)', () => {
  ...
  it('adds aria-label to button with no accessible name', async () => {
    ...
    expect(out).toMatch(/aria-label="Accessible Label"/);
  });

  it('adds <th> to table missing headers', async () => {
    const html = '<table><tr><td>Data</td></tr></table>';
    const violations = [
      { id: 'table-headers', nodes: [{ target: ['table'] }] },
    ];
    const { html: out } = await runAutofix({ html, violations });
    expect(out).toMatch(/<th>Header<\/th>/);
  });

  it('fixes duplicate IDs by making them unique', async () => {
    const html = '<div id="dup"></div><span id="dup"></span>';
    const violations = [
      {
        id: 'duplicate-id',
        nodes: [{ target: ['div#dup'] }, { target: ['span#dup'] }],
      },
    ];
    const { html: out } = await runAutofix({ html, violations });
    // Should have two elements with different IDs now
    const matches = out.match(/id="dup(-[a-z0-9]{6})?"/g);
